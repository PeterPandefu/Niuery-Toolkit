...
| 壁纸 | 必应/Wallhaven 下载、AI 生成（自动去水印）、设为系统桌面壁纸 |
...
└── src-tauri/                    # Tauri 桌面应用（Rust 后端）
    ...
    │   ├── file_saver/           # 文件保存对话框
    │   ├── wallpaper/            # 壁纸下载、AI 生成去水印、设为系统桌面
    │   └── icons/                # 应用图标
    └── tests/                    # Rust 单元测试
```
...
前端通过 Tauri IPC（invoke / event）调用 Rust 后端能力，包括：屏幕捕获、视频录制（FFmpeg）、剪贴板监控、系统资源采集、全局热键、文件对话框、壁纸下载与系统桌面设置等。
