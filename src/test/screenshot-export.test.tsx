...
const { invokeMock, successMock, errorMock } = vi.hoisted(() => ({
  invokeMock: vi.fn(),
  successMock: vi.fn(),
  errorMock: vi.fn(),
}));
...
describe('screenshot editor export and copy', () => {
  ...
  it('reports the exact native save location after exporting', async () => {
    ...
    expect(invokeMock).toHaveBeenCalledWith('save_image_dialog', {
  ...
  it('uses the native clipboard bridge in the desktop app', async () => {
    ...
    expect(invokeMock).toHaveBeenCalledWith('copy_image_to_clipboard', { base64Data: 'encoded-image' });
